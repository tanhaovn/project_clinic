    ...
    methods: {
        ...
        nextStep() {
            ...
            try {
                ...
                const bookingData = {
                    id_bac_si: this.booking.doctorId,
                    ngay_hen: this.booking.date,
                    thoi_gian_bat_dau: this.booking.time,
                    ly_do_kham: this.booking.reason || 'Khám tổng quát'
                };

                // Gọi API đặt lịch thật
                const response = await baseRequestBenhNhan.post('benh-nhan/dat-lich-kham', bookingData, {
                    headers: {
                ...
                if (response.data.status) {
                    // Lấy mã đặt lịch từ response
                    this.bookingCode = response.data.data.ma_dat_lich;
                ...
                alert('Có lỗi xảy ra khi đặt lịch: ' + (error.response?.data?.message || error.message || 'Vui lòng thử lại sau.'));
            } finally {
