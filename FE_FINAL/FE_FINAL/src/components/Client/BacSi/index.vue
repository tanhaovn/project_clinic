                  ...
                  <div class="card-body ">
                      <div class="row ">
                          <div class="col-lg-12 d-flex justify-content-between align-items-center my-3">
                              <div>
                                  <h3 class="text-capitalize fw-bold mb-1">Bác sĩ chuyên gia hàng đầu</h3>
                                  <p class="text-muted fs-6 mb-0">Đội ngũ bác sĩ giàu kinh nghiệm, được đào tạo chuyên sâu
                                      tại các bệnh
                                      viện uy tín</p>
                              </div>
                          </div>
                      ...
                      <div class="row ">
                          <template v-for="(value, idx) in list_bac_si" :key="idx">
                              <div class="col-lg-3 col-md-4 col-sm-6 mb-4 d-flex">
                                  <div class="card border-0 shadow-sm h-100 w-100 rounded-4 overflow-hidden bg-white"
                                      style="transition: transform 0.2s;">
                                      <div class="card-body p-4">
                                          <div class="text-center mb-3">
                                              <img :src="value.hinh_anh" :alt="value.ho_ten"
                                                  class="rounded-circle mb-3"
                                                  style="width: 120px; height: 120px; object-fit: cover;" />
                                              <h5 class="fw-bold mb-1">{{ value.ho_ten }}</h5>
                                              <div class="text-primary mb-2">
                                                <div class="fw-semibold">{{ value.chuc_danh }}</div>
                                                <div class="small">{{ value.ten_chuyen_khoa }}</div>
                                              </div>
                                          </div>
                                          <div class="d-flex justify-content-center gap-4 mb-3">
                                              <div class="text-center">
                                                  <div class="fw-bold text-dark">{{ value.so_nam_kinh_nghiem }}+</div>
                                                  <div class="text-muted small">Năm KN</div>
                                              </div>
                                              <div class="text-center">
                                                  <div class="fw-bold text-dark">{{ value.kinh_nghiem_lam_viec }}</div>
                                                  <div class="text-muted small">Nơi công tác</div>
                                              </div>
                                          </div>
                                          <p class="text-muted small mb-0">{{ value.mo_ta.split('.')[0] }}.</p>
                                      </div>
                                      <div class="card-footer bg-white border-top pt-3">
                                          <div class="d-grid gap-2">
                                              <router-link :to="`/bac-si/chi-tiet/${value.id}`" class="btn btn-primary py-2 fw-semibold">
                                                  <i class="fa-regular fa-calendar-check me-2"></i>Đặt lịch khám
                                              </router-link>
                                              <router-link :to="`/bac-si/chi-tiet/${value.id}`" class="btn btn-outline-primary py-2">
                                                  <i class="fa-regular fa-user me-2"></i>Chi tiết
                                              </router-link>
                                          </div>
                                      </div>
                                  </div>
...
export default {
  ...
  methods: {
      loadData() {
          axios
              .get("http://127.0.0.1:8000/api/client/bac-si/data")
              .then((res) => {
